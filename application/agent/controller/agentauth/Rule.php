...
class Rule extends BackendAgent
{
    ...
    public function _initialize()
    {
        ...
        $this->model = model('AgentAuthRule');
        // 必须将结果集转换为数组
        ...
        if ($this->request->isPost()) {
            ...
            if ($params) {
                ...
                if ($params['pid'] != $row['pid']) {
                    $childrenIds = Tree::instance()->init(collection(AgentAuthRule::select())->toArray())->getChildrenIds($row['id']);
                    if (in_array($params['pid'], $childrenIds)) {
                        $this->error(__('Can not change the parent to child'));
                    }
                }
                //这里需要针对name做唯一验证
                $ruleValidate = \think\Loader::validate('AgentAuthRule');
                $ruleValidate->rule([
