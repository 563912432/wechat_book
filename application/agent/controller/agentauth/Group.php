...
use app\agent\controller\agentAuth\Agent;
...
class Group extends BackendAgent
{
    ...
    /**
     * @var \app\agent\model\AgentAuthGroup
     */
    ...
    {
        parent::_initialize();
        $this->model = model('AgentAuthGroup');

        $this->childrenGroupIds = $this->auth->getChildrenGroupIds(true);

        $groupList = collection(AgentAuthGroup::where('id', 'in', $this->childrenGroupIds)->select())->toArray();
    ...
    public function index()
    {
        if ($this->request->isAjax()) {
            $list = AgentAuthGroup::all(array_keys($this->groupdata));
    ...
    public function add()
    {
        if ($this->request->isPost()) {
            ...
            $parentmodel = model("AgentAuthGroup")->get($params['pid']);
            if (!$parentmodel) {
    ...
    public function edit($ids = null)
    {
        ...
        if ($this->request->isPost()) {
            ...
            if ($params) {
                ...
                try {
                    $row->save($params);
                    $children_auth_groups = model("AgentAuthGroup")->all(['id'=>['in',implode(',',(Tree::instance()->getChildrenIds($row->id)))]]);
                    $childparams = [];
                    ...
                    model("AgentAuthGroup")->saveAll($childparams);
                    Db::commit();
                    $this->success();
                }catch (Exception $e){
                    Db::rollback();
    ...
    public function del($ids = "")
    {
        if ($ids) {
            ...
            $groupaccessmodel = model('AgentAuthGroupAccess');
    ...
    {
        $this->loadlang('agentAuth/group');

        $model = model('AgentAuthGroup');
        ...
        if (($pid || $parentGroupModel) && (!$id || $currentGroupModel)) {
            $id = $id ? $id : null;
            $ruleList = collection(model('AgentAuthRule')->order('weigh', 'desc')->order('id', 'asc')->select())->toArray();
            //读取父类角色所有节点列表
            $parentRuleList = [];
            ...
            //角色组列表
            $groupTree->init(collection(model('AgentAuthGroup')->where('id', 'in', $this->childrenGroupIds)->select())->toArray());
